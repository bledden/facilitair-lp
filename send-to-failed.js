// Send blog announcement to the 4 failed subscribers
...
async function sendToFailed() {
    console.log('Sending to 4 failed subscribers...\n');
    ...
    // Get subscriber data for these emails
    const subscribers = db.prepare(`
        SELECT email, unsubscribe_token
        FROM subscribers
        WHERE email IN (${failedEmails.map(() => '?').join(',')})
        AND confirmed = 1
    `).all(...failedEmails);
